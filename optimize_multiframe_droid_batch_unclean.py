...
def optimize(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, powerful_optimize_dropdown, noise_input, num_inits_input):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        if tracking_loss and step == 0:
            ...
            # fwd_flows_to_save = 
            # fwd_flows_orig, fwd_valids_orig = run_tracker_on_images(cotracker, mujoco_images_target)
            
        if tracking_loss and step % 60 == 0:
            for i in range(1, min(6, mujoco_images_target.shape[0])):
                flow_2d_mujoco = fwd_flows[i-1].permute(1, 2, 0).detach().cpu().numpy()
                flow_2d_np = flow_2d_mujoco  # [H, W, 2]
                ...
                axes[2].imshow(new_image_np)
                axes[2].quiver(x, y, dx - dx_render, dy - dy_render, color='red', angles='xy', scale_units='xy', scale=1, width=0.001)
                axes[2].set_title('Image with Flow Arrows')
                axes[2].axis('off')

                # Step 5: Save the figure as an image
                plt.tight_layout()  # Adjusts spacing to minimize white space
                plt.savefig(f'inspection_features/flow_visualization_compare{i}.png', dpi=300)  # Save as PNG with 300 DPI
                plt.close()
        ...
        if step % 60 == 0:
            for i in range(1, min(6, mujoco_images_target.shape[0])):
                ...
                # Step 5: Save the figure as an image
                plt.tight_layout()  # Adjusts spacing to minimize white space
                plt.savefig(f'inspection_features/flow_visualization_gt{i}.png', dpi=300)  # Save as PNG with 300 DPI
                plt.close()
