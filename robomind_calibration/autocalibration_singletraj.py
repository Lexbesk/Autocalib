...
"""
python robomind_calibration/autocalibration_singletraj.py --model_path output/franka_fr3_2f85_highres_finetune_0 --scene_path /data/group_data/katefgroup/datasets/robomind/robomind_chenyu/auto_calibration/2024_09_20_pick_fruit_and_bread/0920_155013
"""
...
if __name__ == "__main__":
    ...
    for camera_name in camera_names:
        ...
        extrinsics = INIT_W2C[camera_name] # TODO: change this to initialize using previous results in the dict
        image_list_whole = []
        depth_list_whole = []
        mask_list_whole = []
        ...
        # select image ids for optimization
        L = len(image_list_whole)
        skip_step = L // OPTIMIZE_FRAME_NUM
        optimize_image_ids = []
        for i in range(10, L, skip_step):
            if i >= L - 20:
                break
            optimize_image_ids.append(i)
        optimize_image_ids = [(idx, idx+1) for idx in optimize_image_ids] # to use flow, we need pairs
        print(optimize_image_ids, 'optimize image ids')
        
        # Run cotracker to get flow
        cotracker = torch.hub.load("facebookresearch/co-tracker", "cotracker3_offline").to(device, non_blocking=True)
        os.makedirs(os.path.join(DATA_PATH, 'tracker_data'), exist_ok=True)
        for idx, idx_next in tqdm(optimize_image_ids):
            image_pair = image_list_whole[idx:idx_next+1]    
            data_file = os.path.join(DATA_PATH, 'tracker_data', f'tracker_data_{idx}_{idx_next}.pth')
            prepare_flow(image_pair, device, data_file)

        
        # grid search for extrinsics with perturbation
        ...
        for epoch in tqdm(range(epochs)):
            for idx, idx_next in optimize_image_ids:
                ...
                torch.cuda.empty_cache()
            print(params)
            if traj_name not in results_dict:
                results_dict[traj_name] = {}
            results_dict[traj_name][camera_name] = {'extrinsics': params.tolist()}
