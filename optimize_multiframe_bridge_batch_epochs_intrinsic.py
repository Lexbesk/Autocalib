...
def optimize(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, fwd_flows, fwd_valids):
    ...
    for c_idx in range(image_list.shape[0]):
        ...
        fx = torch.tensor(intrinsic_rh[0, 0], dtype=torch.float32)
        fy = torch.tensor(intrinsic_rh[1, 1], dtype=torch.float32)
        fovx = 2 * torch.atan(W / (2 * fx))
        fovy = 2 * torch.atan(H / (2 * fy))
    ...
    for camera in all_cameras[:]:
        ...
        # Optimize pose parameters (w, v) if optimize_camera is True
        if optimize_camera:
            param_groups.append({'params': [camera.w, camera.v], 'lr': camera_lr})
        
        # Optimize focal length parameters (FoVx, FoVy) if optimize_focal is True
        if optimize_focal:
            param_groups.append({'params': [camera.fx], 'lr': focal_lr})
        
        # Create optimizer and scheduler if there are parameters to optimize
        if param_groups:
    ...
    # if optimize_focal:
    #     optimizer = torch.optim.Adam([camera_pose.FoVx, camera_pose.FoVy], lr=focal_lr)
    #     scheduler = StepLR(optimizer, step_size=10, gamma=0.5)
    #     optimizers.append(optimizer)
    #     schedulers.append(scheduler)
...
"""
python optimize_multiframe_bridge_batch_epochs_intrinsic.py --model_path output/widow0
"""
...
if __name__ == "__main__":
    BATCH_PATH = '/data/group_data/katefgroup/datasets/bridge_chenyu/numpy/rss/toykitchen6/pnp_sweep/00/'
    BATCH_PATH = '/data/group_data/katefgroup/datasets/bridge_chenyu/numpy/rss/toykitchen7/pnp_sweep/00'
    BATCH_PATH = '/data/group_data/katefgroup/datasets/bridge_chenyu/numpy/rss/toykitchen2/pnp_push_sweep/00/'
    BATCH_PATH = '/data/group_data/katefgroup/datasets/bridge_chenyu/numpy/rss/tabletop_dark_wood/pnp_sweep/01/'
    ...
    SPLIT_PATHs = [d for d in os.listdir(BATCH_PATH) if os.path.isdir(os.path.join(BATCH_PATH, d))]
    SPLIT_PATHs = sorted(SPLIT_PATHs)
    for i, traj_name in enumerate(SPLIT_PATHs):
        print(i, traj_name)
        # if traj_name not in ['train']:
        #     continue
        SPLIT_PATH = os.path.join(BATCH_PATH, traj_name)
        results_file = os.path.join(SPLIT_PATH, 'results.pkl')
        try:
            results_dict = pickle.load(open(results_file, 'rb'))
        except:
            results_dict = {}
        camera_names = [d for d in os.listdir(SPLIT_PATH) if os.path.isdir(os.path.join(SPLIT_PATH, d))]
        camera_names = sorted(camera_names)
        out = np.load(os.path.join(SPLIT_PATH, 'out.npy'), allow_pickle=True)
        # print(len(out))
        vis_features = True
        tracking_loss = True
        use_sam_mask = False
        ...
        for traj_id in range(len(out)): # for each trajectory
            image_list_whole = []
            states_whole = []
            qpos_whole = []
            out_cur = out[traj_id]
            obs = out_cur["observations"]
            acts = out_cur["actions"]
            # print(acts)
            # print(len(obs))
            for k in range(len(obs)):
                frame = obs[k]
                # print(frame.keys())
                images0 = frame['images0']
                time_stamp = frame['time_stamp']
                state = frame['state']
                qpos = frame['qpos']
                print(qpos.shape, 'qpos shape')
                qpos_whole.append(qpos)
                
                # transform = state2transform(state, DEFAULT_ROTATION)
                # x, y, z, qw, qx, qy, qz = transform2quat(transform)
                # desired_pos = np.array([x, y, z])
                # desired_quat = np.array([qw, qx, qy, qz])
                
                # # joint = state[:6]
                # # desired_pos = np.array(joint[:3])  # Example target position [x, y, z]
                # # rot_euler = Rotation.from_euler('xyz', joint[3:6], degrees=False)
                # # rot_adjust = Rotation.from_rotvec([0, np.pi/2, 0])
                # # rot_target = rot_adjust * rot_euler
                # # q_target_scipy = rot_target.as_quat()
                # # desired_quat = np.array([q_target_scipy[3], q_target_scipy[0], q_target_scipy[1], q_target_scipy[2]])

                # # guess for joint angles
                # initial_q = np.zeros(model.nq)
                # bounds = [(model.jnt_range[i, 0], model.jnt_range[i, 1]) for i in range(model.nq)]
                # result = minimize(objective, initial_q, method='SLSQP', bounds=bounds)
                
                # # bring back the gripper position
                # joint_angles = result.x[:6]
                # state = np.concatenate([joint_angles, state[-1:]], axis=0)
                # print(state, 'state')
                # print(images0.shape, 'images0 shape')
                # print(time_stamp, 'time stamp')
                # print(state.shape, 'state shape')
                image_list_whole.append(images0)
                states_whole.append(state)
                
            image_list_whole = np.stack(image_list_whole)
            qpos_whole = np.stack(qpos_whole)
            print(qpos_whole.shape, 'qpos')
            _, H, W, C = image_list_whole.shape
            states_whole = np.stack(states_whole)
            print(image_list_whole.shape, 'images')
            print(states_whole.shape, 'states')
            
            grippers_whole = states_whole[:, -1:]
            joints_whole = states_whole[:, :-1]
            # print(grippers_whole.shape, 'grippers')
            # print(joints_whole.shape, 'joints')
            # print(grippers_whole.max(), 'grippers max')
            # print(grippers_whole.min(), 'grippers min')
            fingers_whole = grippers_whole * 0.022 + 0.015
            fingers_whole = np.concatenate([fingers_whole, -fingers_whole], axis=1)
            # print(fingers_whole.shape, 'fingers')
            joints_whole = np.concatenate([qpos_whole, fingers_whole], axis=1)
            ...
            data_file = os.path.join(SPLIT_PATH, f'tracker_data_{traj_id}.pth')
            ...
            intrinsics = np.array([[457.52, 0.0, W / 2],
                                [0.0, 457.52, H / 2],
                                [0.0, 0.0, 1.0]])
            f = torch.tensor(intrinsics[0, 0], dtype=torch.float32, requires_grad=True)
            ...
            gt_extrinsics = w2c
            cam_poses = generate_camera_poses('right', num_samples=0)
            best_pose = None
            best_score = float('inf')
            epoch = 0
            ...
            for pose in cam_poses:
                ...
                w2c = np.array([
                                    ...
                                    0.33915317123606575,
                                    0.0],
                                    [-0.025735139945173152,
                                    -0.8080174810137345,
                                    -0.58859617136728104,
                                    0.0],
                                    [-0.12378437639136766,
                                    0.15542215083694305,
                                    0.093593753922678061,
                                    1.0]
                                ]).T
                w2c = np.array([
                                    [0.40230105648431419,
                ...
                joints_lr = 0.0
                optimization_steps = 1
                Optimize_list = [j for j in range(20, 25)]
            ...
            for epoch in tqdm(range(epochs)):
                for batch_idx in range(steps_per_epoch):
                    ...
                    print(first_camera.fx)
                    print('fffffffffffffff')
                    torch.cuda.empty_cache()
                print(params)
                viewpoint_camera = first_camera
                tanfovx = torch.tan(viewpoint_camera.FoVx * 0.5)
                tanfovy = torch.tan(viewpoint_camera.FoVy * 0.5)
                focal_length_x = viewpoint_camera.image_width / (2 * tanfovx)
                focal_length_y = viewpoint_camera.image_height / (2 * tanfovy)
                intrinsics[0, 0] = focal_length_x.item()
                intrinsics[1, 1] = focal_length_y.item()
                print(intrinsics, 'intrinsics')
                results_dict[f'{traj_id}'] = {'extrinsics': params.tolist(), 'valid_optimization': valid_optimization, 'intrinsics': intrinsics.tolist()}
