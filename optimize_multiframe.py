...
python optimize_multiframe.py --model_path output/franka_fr3_2f85_1
...
def optimize(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, powerful_optimize_dropdown, noise_input, num_inits_input, *all_params):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        for camera, joint_pose in zip(all_cameras, all_joint_poses):
            ...
            if FRANKA:
                output = render_gradio(camera, gaussians, background_color, render_features = True)
            elif UR5:
                output = render_gradio(camera, gaussians, background_color, render_features = True)
            ...
            # print(i)
            # print(rendered_feat.shape, 'feature shape') #[1, 32, 360, 640]
        ...
        print(mujoco_direct_feats_target.shape, 'mujoco_feats_target')
        print(mujoco_feats_target.shape, 'mujoco_feats_target')
        ...
        if vis_features:
            for i in [0, -1]:
                visualize_features(gaussian_feats[i].detach().cpu().numpy(), f'inspection_features/features_{i}.png')
                device = "cuda" if torch.cuda.is_available() else "cpu"
                print("Loading DINOv2 model...")
                dinov2 = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
                dinov2 = dinov2.to(device)
                image = torch.cat([gaussian_tensors[i] * 255, mujoco_images_target[i]], dim=1).permute(1, 2, 0)
                ...
                image = resize_image(image_np, 800)
                dino_transform = T.Compose([
                                                T.ToTensor(),
                                                T.Normalize(mean=[0.5], std=[0.5]),
                                            ])
                ...
                features = features.cpu().numpy()
                features_hwc = features.reshape((target_H // dinov2.patch_size, target_W // dinov2.patch_size, -1))
                masked_gaussian_feats = gaussian_feats[i] * gaussian_masks[i].unsqueeze(-1)
                resized_mask = F.interpolate(gaussian_masks[i].unsqueeze(0).unsqueeze(0).float(), size=(features_hwc.shape[:2]), mode='nearest')
                masked_features_hwc = features_hwc * resized_mask.squeeze().unsqueeze(-1).detach().cpu().numpy()
                features_final = F.interpolate(masked_gaussian_feats.unsqueeze(0).permute(0, 3, 1, 2), size=features_hwc.shape[:2], mode="bilinear", align_corners=False).detach().cpu().numpy()
                features_final = features_final.squeeze().transpose(1, 2, 0)
                print(features_hwc.shape, 'features_hwc')
                print(features_final.shape, 'features_final')
                print(masked_features_hwc.shape, 'masked_features_hwc')
                print(resized_mask.shape, 'resize mask')
                features_final = np.concatenate([masked_features_hwc, features_final], axis=0)
                visualize_features(features_final, f'inspection_features/dino_features_{i}.png')
                
                # compute the rendered image's feature and compare to see if there are actually similar
                ...
                with torch.no_grad():
                    features = dinov2.forward_features(image)["x_norm_patchtokens"][0]
                features = features.cpu().numpy()
                features_hwc_render = features.reshape((target_H // dinov2.patch_size, target_W // dinov2.patch_size, -1))
                masked_features_hwc_render = features_hwc_render * resized_mask.squeeze().unsqueeze(-1).detach().cpu().numpy()
                features_final_gt = np.concatenate([masked_features_hwc, masked_features_hwc_render], axis=0)
                visualize_features(features_final_gt, f'inspection_features/gt_dino_features_{i}.png')
...
if __name__ == "__main__":
    ...
    DATA_PATH = '/data/user_data/wenhsuac/chenyuzhang/drrobot/droid1/2023-07-07/Fri_Jul__7_09:45:39_2023/22008760_left'
    DATA_PATH = '/data/user_data/wenhsuac/chenyuzhang/drrobot/droid1/2023-07-07/Fri_Jul__7_10:59:43_2023/22008760_left'
    ...
    print(intrinsics)
    Optimize_list = []
    for ii in range(0, len(image_list), 1):
