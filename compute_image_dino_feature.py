...
# python compute_image_dino_feature.py -s n

def resize_tensor(image, longest_edge=800):
    """
    Resize a PyTorch tensor so that its longest edge equals longest_edge,
    maintaining the aspect ratio, using bilinear interpolation.
    
    Args:
        image (torch.Tensor): Input tensor of shape (C, H, W), float values in [0,1].
        longest_edge (int): Target length for the longest edge (default: 800).
    
    Returns:
        torch.Tensor: Resized tensor of shape (C, H', W').
    """
    # Get current height and width from tensor shape (C, H, W)
    B, C, H, W = image.shape
    
    # Calculate the scaling ratio based on the longest edge
    if W > H:
    ...
    image_resized = F.interpolate(
        image,           # Shape: (1, C, H, W)
        size=(new_H, new_W),          # Target size: (new_height, new_width)
        mode='bilinear',              # Bilinear interpolation
        align_corners=False           # Standard setting for image resizing
    )
...
def main(args):
    ...
    image_path = '/data/user_data/wenhsuac/chenyuzhang/drrobot/rh20t_data/f0461559_task_0083_user_0007_scene_0003_cfg_0005/images/00000.jpg'
    dino_path = '/data/user_data/wenhsuac/chenyuzhang/backup/drrobot/inspection/00000.npy'
