...
def optimize(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, powerful_optimize_dropdown, noise_input, num_inits_input):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        os.makedirs(os.path.join(SPLIT_PATH, 'renders'), exist_ok=True)
        os.makedirs(os.path.join(SPLIT_PATH, 'blends'), exist_ok=True)
        for ii in range(image.shape[0]):
            ...
            image_np.save(os.path.join(SPLIT_PATH, f'renders/{ii+t}.png'))
            ...
            # print(image_np.shape)
            image_np = Image.fromarray(image_np)
            image_np.save(os.path.join(SPLIT_PATH, f'blends/{ii+t}.png'))
...
python render_batch_bridge_batch.py --model_path output/widow0
...
if __name__ == "__main__":
    ...
    for i, traj_name in enumerate(SPLIT_PATHs):
        ...
        for traj_id in range(len(out)): # for each trajectory
            res = results_dict[f'{traj_id}']
            extrinsics = res['extrinsics']
            intrinsics = np.array(res['intrinsics'])
            valid_optimization = res['valid_optimization']
            image_list_whole = []
            states_whole = []
            qpos_whole = []
            out_cur = out[traj_id]
            ...
            if valid_optimization:
                output_video = os.path.join(DATA_PATH, f'video_{traj_id}.mp4')    # Output video file name
            else:
                output_video = os.path.join(DATA_PATH, f'video_{traj_id}_invalid.mp4')
