...
import random
import json
from utils_loc.system_utils import searchForMaxIteration
from scene.dataset_readers import sceneLoadTypeCallbacks
from scene.gaussian_model import GaussianModel
from arguments import ModelParams
from utils_loc.camera_utils import cameraList_from_camInfos, camera_to_JSON
...
class RobotScene:
    ...
    def __init__(self, args : ModelParams, 
                 ...
                 opt_params=None,
                 from_ckpt=False,
                 n_sample_cameras=64, continue_train=False, load_path=None):
        """b
        ...
        if load_iteration and not continue_train:
        ...
        print(opt_params.dataset_path, self.model_path)
        if not os.path.exists(os.path.join(self.model_path, "robot_xml/model.xml")):
            assert os.path.exists(os.path.join(opt_params.dataset_path, "robot_xml/model.xml")), "Robot XML file not found in dataset, something is wrong"
        ...
        # breakpoint()
        print(os.path.join(args.model_path, "robot_xml/scene.xml"))
        chain = build_chain_from_mjcf_path(os.path.join(args.model_path, "robot_xml/model.xml"))
