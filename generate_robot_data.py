...
from utils_loc.mujoco_utils import compute_camera_extrinsic_matrix, compute_camera_intrinsic_matrix
from filelock import FileLock
from utils_loc.mujoco_utils import get_canonical_pose, set_xml_light_params, find_non_collision_pose, save_robot_metadata
...
os.environ["RAY_TMPDIR"] = "/data/user_data/wenhsuac/temp1"
# os.environ["RAY_TMPDIR"] = "temp1"
...
class MujocoActor:
    ...
    def find_ee_pos_fk(self, joint_position):
            print('here!!!')
            transforms = self.chain.forward_kinematics(joint_position)
            return transforms[self.ee_link_name].get_matrix()[0,:3,3]

    def generate_and_save_pc(self, sample_id, args, is_canonical=False, is_test=False, verbose=False, center=False):
        ...
        # print(joint_position, 'joint position')
        # print(joint_position.shape, 'joint position shape')
        print(center)
        camera_params = self.generate_camera_params() 
        # center = False
        if center:
            try:
                self.chain = build_chain_from_mjcf_path(self.model_xml_path)
                # print(self.chain)
                self.ee_link_name = 'hand' # for UR5e-robotiq

                ee_pos = self.find_ee_pos_fk(joint_position)
                print(ee_pos)
                # assert abs(ee_pos[0]) < 0.5 and abs(ee_pos[1]) < 0.5 and ee_pos[2] < 0.5
                images, depth_images, intrinsic_matrices, extrinsic_matrices = self.render_images(joint_position, 
                                                                                          camera_params,
                                                                                          render_depth=True,
                                                                                          lookat=ee_pos)
            except:
                print('failed, using: ', [0, 0, 0])
                images, depth_images, intrinsic_matrices, extrinsic_matrices = self.render_images(joint_position, 
                                                                                          camera_params, 
                                                                                          render_depth=True,
                                                                                          lookat=[0, 0, 0.3])
    ...
    def generate_camera_params(self): #this can be implemented in other ways
        ...
        elevations = [45, 45]
        radii = np.array([1.1, 1.1]) * args.camera_distance_factor
        ...
        radius_offset = np.random.uniform(-0.01, 0.01) * args.camera_distance_factor
...
def generate_data(num_actors, num_samples, model_xml_dir, save_dir, args, is_canonical=False, is_test=False, verbose=False):
    ...
    for i in range(num_samples):
        ...
        if i >= 0:
            ...
            center = False
        # center = False
        task = actors[actor_index].generate_and_save_pc.remote(i, args, is_canonical, is_test, True, center=center)
        tasks.append(task)
    ...
    while True:
        ...
        # num_files = len(glob.glob(os.path.join(save_dir, f"{sample_type_prefix}sample_*"))) original
        files = glob.glob(os.path.join(save_dir, f"{sample_type_prefix}sample_*"))
        num_files = len([f for f in files if not f.endswith('.lock')])
        if num_files % 1000 == 0:
            original_num = len(glob.glob(os.path.join(save_dir, f"{sample_type_prefix}sample_*")))
...
def clean_lock_files(directory):
    print('cleaning lock files in', directory)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_canonical_samples', type=int, default=5, help='Number of canonical samples.')
    parser.add_argument('--num_samples', type=int, default=5, help='Number of samples.')
    parser.add_argument('--num_test', type=int, default=0, help='Number of test samples.')
