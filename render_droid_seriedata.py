...
python render_droid_seriedata.py --model_path output/franka_fr3_2f85_complement_1 --scene_path /data/group_data/katefgroup/datasets/droid_chenyu/droid_extract_3/scene_27
...
def optimize(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, powerful_optimize_dropdown, noise_input, num_inits_input):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        for camera, joint_pose in zip(all_cameras, all_joint_poses):
            ...
            print(mask_tensor.shape, 'mask_tensor')
        ...
        os.makedirs(os.path.join(DATA_PATH, 'masks_render'), exist_ok=True)
        for ii in range(image.shape[0]):
            image_np_render = (image[ii] * 255).permute(1, 2, 0).to(torch.uint8)
            mask_np_render = (gaussian_masks[ii] * 255).to(torch.uint8)
            image_np = image_np_render.detach().cpu().numpy()
            mask_np = mask_np_render.detach().cpu().numpy()
            # print(image_np.shape)
            image_np = Image.fromarray(image_np)
            mask_np = Image.fromarray(mask_np, mode='L')  # 'L' for grayscale
            image_np.save(os.path.join(DATA_PATH, f'renders/{ii+t}.png'))
            mask_np.save(os.path.join(DATA_PATH, f'masks_render/{ii+t}.png'))
...
if __name__ == "__main__":
    ...
    # print(results_dict)
    # camera_names = results_dict.keys()
    ...
    if UR5 or ROBOTIQ:
        print(grippers.shape, 'grippers', grippers.max(), grippers.min())
        grippers = np.expand_dims(grippers, axis=1)
        fingers = grippers * np.array([[0.75, -0.4, 0.6, -0.26, 0.75, -0.4, 0.6, -0.26]])
        print(fingers.shape, 'grippers')
        # grippers = grippers * 0
...
# print(all_params[:, -1])
    ...
    output_video = os.path.join(DATA_PATH, f'video_blend.mp4')   
...
#     DATA_PATH = DATA_PATH
#     folder_path = os.path.join(DATA_PATH, 'blends')  # Replace with your folder path
#     output_video = os.path.join(DATA_PATH, f'video_blend_orig.mp4')
