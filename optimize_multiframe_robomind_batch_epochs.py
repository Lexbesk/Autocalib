...
def optimize(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, fwd_flows, fwd_valids):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        print(DATA_PATH, 'epoch:', epoch, Optimize_list)
...
"""
python optimize_multiframe_robomind_batch_epochs.py --model_path output/franka_fr3_2f85_highres_finetune_0 --scene_path None
"""
...
if __name__ == "__main__":
    ...
    for i, traj_name in enumerate(TRAJ_PATHs):
        print(i, traj_name)
        if traj_name not in ['0920_143123']:
            continue
        ...
        for j, cam_name in enumerate(camera_names):
            ...
            assert(image_list_whole.shape[0] == grippers_whole.shape[0] == joints_whole.shape[0])
            ...
            # extrinsics = np.eye(4)
            print(intrinsics)
            cam_poses = generate_camera_poses(cam_name.split('_')[1], num_samples=1)
            best_pose = None
            best_score = float('inf')
            epoch = 0
            ...
            for pose in cam_poses:
                ...
                joints_lr = 0.0
                optimization_steps = 1
                Optimize_list = [j for j in range(15)]
