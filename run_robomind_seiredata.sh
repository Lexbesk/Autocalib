...
BATCH_PATH="/data/group_data/katefgroup/datasets/robomind/robomind_chenyu/robomind_extract_1"
...
for scene_name in "${SCENE_NAMES[@]}"; do
    ...
    if [ "$i" -ge "$START_INDEX" ]; then
        ...
        conda deactivate
        conda activate dr_test
        cd /data/user_data/wenhsuac/chenyuzhang/backup/drrobot

        # Call Python script with --scene_path argument
        python optimize_multiframe_robomind_single_seriedata_fast.py --model_path output/franka_fr3_2f85_highres_finetune_0 --scene_path "$scene_path"
    fi
    ((i++))
done