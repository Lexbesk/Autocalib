...
"""
python optimize_multiframe_bridge_batch_epochs.py --model_path output/widow0
"""
...
if __name__ == "__main__":
    ...
    BATCH_PATH = '/data/group_data/katefgroup/datasets/bridge_chenyu/numpy/bridge_data_v2/datacol1_toykitchen1/many_skills/0'
    ...
    for i, traj_name in enumerate(SPLIT_PATHs):
        ...
        if len(out) == 0:
            print('no out')
            continue
        for traj_id in range(min(1, len(out))): # for each trajectory
            image_list_whole = []
            states_whole = []
            qpos_whole = []
            out_cur = out[traj_id]
            ...
            intrinsics = np.array([[557.5204, 0.0, W / 2],
                                [0.0, 557.5204, H / 2],
                                [0.0, 0.0, 1.0]])
            cam_poses = generate_camera_poses('right', num_samples=100)
